#ifdef getenv("DEVICE")=="PIC18F2455" || getenv("DEVICE")=="PIC18F2550" || getenv("DEVICE")=="PIC18F4455" || getenv("DEVICE")=="PIC18F4550"  // ***** Configuración para la familia de PICs 18Fxx5x.

   #define   RF_CS       PIN_C1     // Patillas del PIC que controla el nRF24L01.
   #define   RF_CE       PIN_C2
   #define   SCK         PIN_B1
   #define   SDI         PIN_B0
   #define   SDO         PIN_C7 
   ...
   #define   RF_CS_TRIS  TRISC,1    // Bits TRIS I/O.
   ...
   void RF_INT_EN()                 // Habilitar interrupción RB2 externa con flanco de bajada.
   {
...
#if getenv("DEVICE")=="PIC16F873A" || getenv("DEVICE")=="PIC16F874A" || getenv("DEVICE")=="PIC16F876A" || getenv("DEVICE")=="PIC16F877A" // ***** Configuración para la familia de PICs 16F87XA.
   #define   RF_IRQ      PIN_B0     // Patillas del PIC que controla el nRF24L01.
   #define   RF_CS       PIN_C1
   #define   RF_CE       PIN_C2
   #define   SCK         PIN_C3
   #define   SDI         PIN_C4
   ...
   #define   RF_IRQ_TRIS TRISB,0    // Bits TRIS I/O.       
   ...
   #BYTE TRISA     =  0x85          // Dirección de los TRIS A, B, C.
   #BYTE TRISB     =  0x86
   #BYTE TRISC     =  0x87
   
   #BYTE INTCON    =  0x0B          // Registro de interrupciones. Sólo es importante el bit 7, GIE=interrupciones globales.
   
   void RF_INT_EN()                 // Habilitar interrupción externa RB0 con flanco de bajada.
   {
      disable_interrupts(global);
...
//
//
//   A partir de aquí la librería sigue casi igual que en la original: "lib_rf2gh4_10.h"
//   con excepción de alguna líneas, casi al final de este archivo.
//
//**************************************************************************************
...
void RF_CONFIG(int canal, int dir)
{
   ...
   //RF_SETUP
   //Configuración aspectos RF. ******************* Configurado para máximo alcance = 40 metros ***************************
   //Ganancia máxima de LNA, 0dBm potencia de salida y 1Mbps de velocidad. ************************************************
   output_low(RF_CS);
   spi_write(0x26);
   spi_write(0x07);    // Lo he cambiado a 1Mbps para máxima distancia. Originalmente era 0x0F (2Mbps, unos 7 metros máximo en espacio abierto).
   output_high(RF_CS);
...
int RF_SEND()
{
   ...
   if(noRF==500)
   {
        if(interRF==1)
        enable_interrupts(GLOBAL);
        #if getenv("DEVICE")=="PIC18F2455" || getenv("DEVICE")=="PIC18F2550" || getenv("DEVICE")=="PIC18F4455" || getenv("DEVICE")=="PIC18F4550"  // ***** Configuración para la familia de PICs 18Fxx5x.
